// **********************************************************************
//
// Copyright (c) 2003-2009 CDE, Inc. All rights reserved.
//
// This copy of Cde is licensed to you under the terms described in the
// CDE_LICENSE file included in this distribution.
//
// **********************************************************************

// CDE version 1.0.1

package Message.Public{

import Framework.Serialize.SerializeStream;
import Framework.Util.StringFun;
import Framework.Util.Exception;
import Framework.MQ.*;
import Framework.Holder.*;

import Engine.RMI.*;
import flash.utils.ByteArray;

import flash.utils.Dictionary;


public class SPublicPlayer extends IMessageBase 
{
    public var entityId : SEntityId;

    public var name : String;

    public var guildName : String;

    public var sex : int;

    public var career : int;

    public var camp : int;

    public var level : int;

    public var VIP : int;

    public var avatar : int;

    public var combat : int;

    public var gateChannelId : int;

    public var mapId : int;

    public var copyCode : int;

    public var copyId : int;

    public var guildId : int;

    public var online : Boolean;

    public var alive : int;

    public function SPublicPlayer(reg : Boolean = false)
    {
        if( reg )
        {
            MessageManager.instance().regist( this );
        }
    }

    public static var _regist:SPublicPlayer = new SPublicPlayer( true );

    override public function getType() : int
    {
        return _type;
    }

    public const _type : int = 81;

    override public function clone() : IMessageBase
    {
        return new SPublicPlayer;
    }

    override public function __write( __os : SerializeStream ) : void
    {
        entityId.__write(__os);
        __os.writeString(name);
        __os.writeString(guildName);
        __os.writeByte(sex);
        __os.writeShort(career);
        __os.writeByte(camp);
        __os.writeByte(level);
        __os.writeByte(VIP);
        __os.writeInt(avatar);
        __os.writeInt(combat);
        __os.writeInt(gateChannelId);
        __os.writeInt(mapId);
        __os.writeInt(copyCode);
        __os.writeInt(copyId);
        __os.writeInt(guildId);
        __os.writeBool(online);
        __os.writeShort(alive);
    }

    override public function __read( __is : SerializeStream ) : void 
    {
        entityId = new SEntityId();
        entityId.__read(__is);
        name = __is.readString();
        guildName = __is.readString();
        sex = __is.readByte();
        career = __is.readShort();
        camp = __is.readByte();
        level = __is.readByte();
        VIP = __is.readByte();
        avatar = __is.readInt();
        combat = __is.readInt();
        gateChannelId = __is.readInt();
        mapId = __is.readInt();
        copyCode = __is.readInt();
        copyId = __is.readInt();
        guildId = __is.readInt();
        online = __is.readBool();
        alive = __is.readShort();
    }
}
}
