 ...
 */
	public class TweenCore {
		...
		public function TweenCore(duration:Number=0, vars:Object=null) {
			...
			this.frameInterval = (this.vars.frameInterval) ? int(this.vars.frameInterval) : 2;
...
//			}
		 ...
		 */
		public function renderTime(time:Number,frameCount:int,suppressEvents:Boolean=false, force:Boolean=false):void {
		...
		public function complete(skipRender:Boolean=false, suppressEvents:Boolean=false):void {
			if (!skipRender) {
				renderTime(this.totalDuration,-1, suppressEvents, false); //just to force the final render
		...
		protected function setTotalTime(time:Number, suppressEvents:Boolean=false):void {
			if (this.timeline) {
				...
				if (this.cachedTotalTime != time) {
					renderTime(time, -1,suppressEvents, false);
				}
			}
		}
