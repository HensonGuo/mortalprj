	...
	public class SimpleTimeline extends TweenCore {
		...
		override public function renderTime(time:Number, frameCount:int,suppressEvents:Boolean=false, force:Boolean=false):void {
			...
			while (tween) {
				...
				if ((frameCount == -1 || frameCount%tween.frameInterval ==0) && (tween.active || (time >= tween.cachedStartTime && !tween.cachedPaused && !tween.gc))) {
					if (!tween.cachedReversed) {
						tween.renderTime((time - tween.cachedStartTime) * tween.cachedTimeScale, frameCount,suppressEvents, false);
					} else {
						dur = (tween.cacheIsDirty) ? tween.totalDuration : tween.cachedTotalDuration;
						tween.renderTime(dur - ((time - tween.cachedStartTime) * tween.cachedTimeScale),frameCount, suppressEvents, false);
					}
				}
				tween = next;
			}
