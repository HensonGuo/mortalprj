package mortal.game.view.common.richTextField.richTextInput
{
	 ...
	 * <listing>
		...
		&lt;/rtf&gt;</listing>
	 */
	public class RichTextInput extends Sprite
	{
		 ...
		 * 
		 */		
		public function exportToCellData():Array
		{
			var xml:XML =<rtf/>;
			if (html) 
			{
		...
		private function changeToCellData(data:XML):Array
		{
			...
			var sprites:Array = [];
			var length:int = data.sprites.sprite.length();
			...
			var startIndex:int;
			for(var n:int ; n < sprites.length ; n++)
			{
				var spriteObj:DisplayObject = getSpriteFromObject(sprites[n].src);;
				var mc:Object = {};
				var txt:Object = {};
				if(sprites[n].index > startIndex)
				{
					txt = {type:"text",text:content.slice(startIndex,sprites[n].index)}
					arr.push(txt);
					mc = {type:"movieClip",data:spriteObj};
					arr.push(mc);
					startIndex = sprites[n].index;
				}
//				else if(sprites[n].index == startIndex)
//				{
//					obj = {type:"movieClip",data:_smileys[sprites[n].scr]};
//					arr.push(obj);
//				}
				else if(sprites[n].index <= startIndex)
				{
			...
			if(content.slice(startIndex) != "")
			{
				var obj3:Object = {type:"text",text:content.slice(startIndex)}
				arr.push(obj3);
			}
			return arr;
		}
