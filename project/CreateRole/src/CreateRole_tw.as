package
{
	...
	public class CreateRole_tw extends MovieClip 
	{
		...
		private var _createRoleMC:CreateRoleMC_tw;
		...
		private function onAddToStage(event:Event):void
		{
			...
			_createRoleMC = new CreateRoleMC_tw();
			...
			_createNameTxt.htmlText = "<a href='event:0'><u>隨機取名</u><a>";
			...
			_randomBtn = _createRoleMC.randomBtn;
		...
		private function onMouseClickHandler( event:MouseEvent ):void
		{
			...
			else if(target.name == "randomBtn")//随机名字
			{
		...
		private function checkTipPanel():Boolean
		{
			...
			else if(!nameExp.test(nameTxt))
			{
				_tipPanel.x = _roleNameTxt.x + _roleNameTxt.width;
				_tipPanel.y = _roleNameTxt.y + _roleNameTxt.height/2;
				_tipPanel.updateTips("輸入的角色名含非法字符，如&，#，@");
				_tipPanel.visible = true;
				return false;
			}
			else if(nameTxt.length < 2)
			{
				_tipPanel.x = _roleNameTxt.x + _roleNameTxt.width;
				_tipPanel.y = _roleNameTxt.y + _roleNameTxt.height/2;
				_tipPanel.updateTips("角色名必須為2~7个字符");
				_tipPanel.visible = true;
				return false;
			}
			else
		...
		{
			_roleNameTxt.text = RandomName_tw.getName( _currentSex );
			checkTipPanel();
		}
